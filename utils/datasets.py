...
class HGCDataset(GCDataset):
    ...
    def compute_high_next_idxs(self, idxs, final_state_idxs, high_goal_idxs, subgoal_steps):
        ...
        subgoal_steps = np.minimum(subgoal_steps, final_state_idxs + 1 - idxs)
    ...
    def sample(self, batch_size: int, idxs=None, evaluation=False):
        ...
        # add option rewards for high value
        batch['high_value_option_rewards'] = high_value_successes - (1.0 if self.config['gc_negative'] else 0.0)
...
@dataclasses.dataclass
class HGCDataset_ota(GCDataset_ota):
    """Dataset class for hierarchical goal-conditioned RL.
    ...
    def sample(self, batch_size, idxs=None, evaluation=False):
        ...
        if 'ota' in self.config['agent_name']:
