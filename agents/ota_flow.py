...
class ActorVectorField(nn.Module):
    ...
    def __call__(
        ...
        `goal_encoded` matches GCActor semantics: observations still go through the state encoder, while goals are
        treated as already-encoded representations.
        """
        if self.gc_encoder is not None and not is_encoded:
...
class OTAAgent_flow(flax.struct.PyTreeNode):
    """Hierarchical implicit Q-learning (HIQL) agent.

    Changes:
        - replace gaussian policy with flow policy.
    """
    ...
    def low_actor_loss(self, batch, grad_params, rng=None):
        ...
        is_goal_encoded = self.config['use_value_rep'] or self.config['use_actor_rep']
        pred = self.network.select('low_actor_flow')(
            batch['observations'], goal_reps, x_t, t, goal_encoded=is_goal_encoded, params=grad_params
        )

        actor_loss = jnp.mean(exp_a * (pred - y) ** 2)
    ...
    def sample_actions(
        ...
        batch_shape = observations.shape[:-3] if self.config['encoder'] is not None else observations.shape[:-1]

        if subgoals is None:
            goal_reps = jax.random.normal(high_seed, (*batch_shape, self.config['rep_dim']))
            for i in range(self.config['flow_steps']):
                t = jnp.full((*batch_shape, 1), i / self.config['flow_steps'])
                vels = self.network.select('high_actor_flow')(observations, goals, goal_reps, t)
                goal_reps = goal_reps + vels / self.config['flow_steps']
            if self.config['use_value_rep'] or self.config['use_actor_rep']:
        ...
        actions = jax.random.normal(low_seed, (*batch_shape, self.config['action_dim']))
        for i in range(self.config['flow_steps']):
            t = jnp.full((*batch_shape, 1), i / self.config['flow_steps'])
            is_goal_encoded = self.config['use_value_rep'] or self.config['use_actor_rep']
            vels = self.network.select('low_actor_flow')(
                observations, goal_reps, actions, t, goal_encoded=is_goal_encoded
            )
    ...
    def create(
        ...
        # if use goal_rep, then ex_subgoals will be subgoal representations.
        ex_subgoals = example_batch['high_actor_targets']
        if config['use_value_rep'] or config['use_actor_rep']:
            ex_subgoals = jnp.zeros((ex_subgoals.shape[0], config['rep_dim']))

        assert config['use_value_rep'] or config['use_actor_rep'], "At least one of them must be True."

        if config['use_value_rep']:
        ...
        network_info = dict(
            ...
            high_actor_flow=(high_actor_flow_def, (ex_observations, ex_goals, ex_subgoals, ex_times)),
        )
        if config['use_value_rep'] or config['use_actor_rep']:
            network_info.update(
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='ota_flow',  # Agent name.
