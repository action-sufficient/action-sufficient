...
class ButtonPlanOracle(PlanOracle):
    ...
    def reset(self, ob, info, target_button=None):
        if target_button is None:
            ...
            # manually set the target button
            env = self._env.unwrapped
            button_site_id = env._button_site_ids[target_button]
            ...
            plan_input = {
                ...
                'effector_goal': self.to_pose(
                    pos=np.append(info['privileged/target_button_top_pos'][:-1], 0.25), # right above the target button
                    # pos=np.random.uniform(*self._env.unwrapped._arm_sampling_bounds),
                    yaw=info['proprio/effector_yaw'][0]
                    # yaw=np.random.uniform(-np.pi, np.pi),
                ),
