...
class CubePlanOracle(PlanOracle):
    ...
    def reset(self, ob, info, target_block=None):
        if target_block is None:
            ...
            # manually set the target block
            env = self._env.unwrapped
            target_mocap_id = env._cube_target_mocap_ids[target_block]
            info['privileged/target_block_pos'] = env._data.mocap_pos[target_mocap_id].copy()
            info['privileged/target_block_yaw'] = np.array(
                [lie.SO3(wxyz=env._data.mocap_quat[target_mocap_id]).compute_yaw_radians()]
            )
            plan_input = {
                'effector_initial': self.to_pose(
                    pos=info['proprio/effector_pos'],
                ...
                'effector_goal': self.to_pose(
                    pos=np.append(info['privileged/target_block_pos'][:-1], 0.25), # right above the target block
                    # pos=np.random.uniform(*self._env.unwrapped._arm_sampling_bounds), [[ 0.25 -0.35  0.2 ], [ 0.6   0.35  0.35]]
                    yaw=info['privileged/target_block_yaw'][0],
                    # yaw=np.random.uniform(-np.pi, np.pi),
                ),
                'block_initial': self.to_pose(
