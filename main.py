...
from utils.datasets import Dataset, GCDataset, HGCDataset, HGCDataset_ota
from utils.evaluation import evaluate, evaluate_low_level
...
flags.DEFINE_string('wandb_mode', 'disabled', 'WandB mode (online, offline, disabled).')
flags.DEFINE_string('wandb_entity', None, 'Optional W&B entity.')
flags.DEFINE_string('wandb_project', 'action-sufficient-rep', 'W&B project name.')
...
flags.DEFINE_integer('offline_steps', 2500000, 'Number of offline steps.')
flags.DEFINE_integer('log_interval', 10000, 'Logging interval.')
flags.DEFINE_integer('eval_interval', 500000, 'Evaluation interval.')
flags.DEFINE_integer('save_interval', 2500000, 'Saving interval.')

flags.DEFINE_boolean('evaluate_low_level', False, 'Whether to evaluate low-level policy.')
flags.DEFINE_integer('low_level_subgoal_steps', 10, 'Number of steps between subgoals for low-level evaluation.')
...
def main(_):
    ...
    dataset_class_dict = {
        ...
        'HGCDataset_ota': HGCDataset_ota,  # for OTA
    }
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if FLAGS.eval_interval != 0 and (i == 1 or i % FLAGS.eval_interval == 0):
            ...
            for k, v in overall_metrics.items():
                ...
                if k == 'success':
                    overall_success_max = max(overall_success_max, float(np.mean(v)))
            ...
            if FLAGS.evaluate_low_level:
                ...
                for task_id in tqdm.trange(1, num_tasks + 1):
                    ...
                    eval_info, trajs, cur_renders = evaluate_low_level(
                        agent=agent,
                        env_name=FLAGS.env_name,
                        task_id=task_id,
                        config=config,
                        ...
                        eval_temperature=FLAGS.eval_temperature,
                        eval_gaussian=FLAGS.eval_gaussian,
                        low_level_subgoal_steps=FLAGS.low_level_subgoal_steps,
                    )
                    renders.extend(cur_renders)
                    metric_names = ['success']
                    eval_metrics.update(
                        {f'evaluation_low_level/{task_name}_{k}': v for k, v in eval_info.items() if k in metric_names}
                    )
                    for k, v in eval_info.items():
                        if k in metric_names:
                            overall_metrics[k].append(v)
                for k, v in overall_metrics.items():
                    eval_metrics[f'evaluation_low_level/overall_{k}'] = np.mean(v)
    ...
    print(f'Maximum overall success rate: {overall_success_max:.6f}')
